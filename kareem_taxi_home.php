...
function retrieve_all_countries() 
{
  ...
  $query = "SELECT  country.id , country.name , group_concat(concat(location.id,':',location.name,':',location.latitude,':',location.longitude)) as location_detail
      ...
      WHERE country.enable = '0' GROUP BY country.id";
...
$query = " SELECT captain.total_captains , 
      MIN(ROUND(SQRT(
      ...
      country.percentage as company_percentage  
      FROM Tracking as tracking
      
      INNER JOIN (
    
        SELECT   cp.* , GROUP_CONCAT(cp.id) as total_captains      
        FROM Captain as cp WHERE cp.enable = '0' AND cp.status = '0' AND cp.live = '0' 
        AND ( SELECT COUNT(*) FROM RideOrder WHERE status = '1' AND captain_id = cp.id) <= 0 GROUP BY cp.ride_type_id
    

      ) captain ON captain.ride_type_id =  '$ride_type_id'
      
      INNER JOIN RideType as ride_type ON ride_type.id = captain.ride_type_id AND ride_type.enable = '0' 
      INNER JOIN Location as location ON location.id = ride_type.place_id AND location.enable = '0' 
      INNER JOIN Country as country ON country.id = location.country_id AND country.enable = '0' 
      GROUP BY captain.total_captains
      HAVING distance <= '2'  ";

       $result=queryRunner($query);
       while ($row = rowRetrieverObject($result)) { 
           
           $total_captains = array_filter(explode(",",$row -> total_captains));
          if(count($total_captains)>1){
              $captain_id =  $total_captains[rand(0,count($total_captains))];
          }
          elseif(count($total_captains)==1){
              $captain_id =  $total_captains[0];
          }
          else{
              $captain_id = "0";
          }
